#!/usr/bin/env python3
"""
Sugarscape Structural Ablation â€” Autoresearch Pipeline
...
Architecture (3-file Karpathy pattern):
  - prepare.py  (FIXED) â€” baseline runner, evaluation harness
  - strategy.py (MUTABLE) â€” the code the LLM simplifies
  - program.md  (HUMAN)  â€” research agenda for the LLM

Usage:
    export GOOGLE_API_KEY="your_key_here"
    python sugarscape_autoresearch.py

The loop runs indefinitely â€” kill with Ctrl+C.
"""
...
# ---------------------------------------------------------------------------
# LLM Agent â€” Gemma via google-genai
# ---------------------------------------------------------------------------
...
def generate_ablation_variants(
    ...
    Learn from the results history â€” if a simplification caused a metric to fail,
...
def calibrate_baseline():
    ...
    log("â•‘  Calibrating Against Mesa Canonical Sugarscape  â•‘")
    ...
    # No cached baseline â€” run Mesa canonical to establish ground truth
...
def run_ablation_round(generation: int, client: genai.Client, baseline: dict):
    ...
    log(f"\nâ•â•â• Generation {generation} â•â•â•")
    ...
    for i, (code, desc) in enumerate(variants):
        ...
        if passes:
            log(f"  âœ… PASSES all metric bounds!")
    ...
    if best_variant:
        ...
        if score_saved > 0:
            log(f"\nðŸŽ‰ RATCHET FORWARD: {desc}")
            log(f"   Score: {current_complexity['combined_score']:.1f} â†’ {complexity['combined_score']:.1f} ({score_saved:+.1f})")
            log(f"   Lines: {current_complexity['lines']} â†’ {complexity['lines']}")
            ...
            log(f"\nâ¸ï¸  Variant passes but not simpler â€” skipping commit")
            return False
    else:
        log(f"\nâŒ No passing variants this generation â€” reverting")
...
def main():
    log("â•”â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•—")
    log("â•‘  Sugarscape Structural Ablation Pipeline        â•‘")
    log(f"â•‘  Model: {MODEL:40s} â•‘")
    log("â•‘  Pattern: Karpathy Autoresearch Ratchet Loop    â•‘")
    log("â•šâ•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•")
