"""
Sugarscape Parameter Sweep Validation ΓÇö Epstein & Axtell (1996)
...
def run_condition(params: dict, n_runs: int = N_SWEEP_RUNS, label: str = "") -> dict:
    ...
    if not passes:
        log(f"  Γ¥î {label} diverges from Mesa baseline!")
        ...
        log(f"  Γ£à {label} matches Mesa baseline!")
...
def format_comparison(comp: dict) -> str:
    ...
    if "error" in comp:
        return f"  {comp.get('metric', '?')}: ERROR ΓÇö {comp['error']}"
...
def sweep_a_trade(n_runs: int = N_SWEEP_RUNS) -> dict:
    ...
    log(f"\nValidations: {validations}")
    log(f"Sweep A {'PASSES Γ£à' if result['all_valid'] else 'FAILS Γ¥î'}")
...
def sweep_b_vision(n_runs: int = N_SWEEP_RUNS) -> dict:
    ...
    Paper prediction:
      - Higher vision ΓåÆ higher resource extraction efficiency
      - Higher vision ΓåÆ higher carrying capacity
      - Higher vision ΓåÆ higher Gini (wealth inequality)
    """
    log("=========================================================")
    ...
    log(f"\nValidations: {validations}")
    log(f"Sweep B {'PASSES Γ£à' if result['all_valid'] else 'FAILS Γ¥î'}")
...
def sweep_c_density(n_runs: int = N_SWEEP_RUNS) -> dict:
    ...
    log(f"\nValidations: {validations}")
    log(f"Sweep C {'PASSES Γ£à' if result['all_valid'] else 'FAILS Γ¥î'}")
...
def sweep_d_scarcity(n_runs: int = N_SWEEP_RUNS) -> dict:
    ...
    Paper prediction:
      - High metabolism / slow regrowth ΓåÆ faster starvation
    ...
    as halving regrowth ΓÇö agents consume faster relative to supply).
    """
    log("=========================================================")
    ...
    for comp_name, comp in comparisons.items():
        log(format_comparison(comp))
    log(f"\nValidations: {validations}")
    log(f"Sweep D {'PASSES Γ£à' if result['all_valid'] else 'FAILS Γ¥î'}")
...
def run_all_sweeps(n_runs: int = N_SWEEP_RUNS) -> dict:
    ...
    for key, r in results.items():
        status = "Γ£à PASS" if r["all_valid"] else "Γ¥î FAIL"
        log(f"  Sweep {key}: {status}")
    log(f"\nOverall: {'Γ£à ALL SWEEPS PASS' if all_valid else 'Γ¥î SOME SWEEPS FAIL'}")
